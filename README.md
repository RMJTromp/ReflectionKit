...
abstract class Player {
    ...
    public void teleport(Object player, boolean legacy) {
        if (legacy) {
            legacyTeleport(player.location);   // only resolves "teleportTo" if called
        } else {
            newerTeleport(player);    // only resolves "teleportPlayer" if called
        }
    }
}
...
The same pattern works with `MappedBase`, using `call()` for direct method invocation:

```java
@Proxy
abstract class Player extends MappedBase {

    // MappedBase helpers give direct access to the target
    public void teleport(Object player) {
        if (legacy) {
            call("teleportTo", player.location); // call target method directly by name
        } else {
            call("teleportPlayer", player);
        }
    }
}
```
